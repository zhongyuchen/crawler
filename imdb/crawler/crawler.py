...
def scrape_top_250(soup):
    """
    Scrape the IMDB top 250 movies index page for target urls
    Args:
    ...
    """
    movie_urls = []
    # LOOK FOR THE ABSOLUTE URLS TO EACH MOVIE'S IMDB PAGE, ADD THOSE TO THE LIST movie_urls.
...
def get_names(staff):
    """
    Find all the names in the dom and combine all the names into a string.
    Args:
        a staff dom
    Returns:
        a string name
...
def get_minutes(duration):
    """
    Transform time string into int minutes
    Args:
        a string time, e.g. "2h", "2h 40min"
    Returns:
        the total minutes
    """
...
def scrape_movie_page(dom):
    """
    Scrape the IMDB page for a single movie through the url
    Args:
